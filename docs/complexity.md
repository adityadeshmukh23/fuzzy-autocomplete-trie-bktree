...
|---|---:|---:|---:|---:|
| prefix, 6-char query | 8.6× | 120× | 317× | **445×** |
| fuzzy, distance 1 | 1.6× | 1.8× | 4.2× | **3.5×** |
| fuzzy, distance 2 | 0.82× | 0.74× | 1.2× | **0.93×** |

The trie changes the complexity class: 0.37 µs at 1,000 words and 2.56 µs at 100,000, against a
...
**These numbers are from a clean run:** all 56 measurements passed the variance guard (0 flagged),
against 13 of 28 flagged on the earlier contended run. Machine load is the single biggest source
of noise in these figures, which is why the guard exists.
